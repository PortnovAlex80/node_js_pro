...
| |1 Приемка/отгрузка товара (Update quantity)  |
...
```mermaid
graph LR
    subgraph "Система управления запасами и складом"
        WMS
    end
    Интернет_магазин -->|Каталог и остатки| WMS
    Складские_работники -->|Изменение остатков, информация о товарах| WMS
    Менеджеры_склада -->|Управление каталогом, данные об остатках, планы выполнения заказов| WMS
    Администраторы_WMS -->|Данные пользователей, настройки доступа| WMS
    Логистика -->|Графики выполнения заказов| WMS
    Telegram_Бот -->|Данные об остатках| WMS
    REST_API -->|Данные для интеграции| WMS

```

```mermaid
graph LR
    subgraph Users["Группа пользователей системы WMS"]
        MGR["Менеджер склада"]
        ADMIN["Администратор"]
        EMPLOYEE["Работник склада"]
    end
    
	MGR--> |Информация о новом товаре, График сборки заказов|id1((Система управления складом WMS))
	id1((Система управления складом WMS)) -->|Информация о товарах, Заявки на сборку| MGR
	
	EMPLOYEE-->|Данные о измененном кол-ве товара|id1((Система управления складом WMS))
    id1((Система управления складом WMS)) -->|Информация о кол-ве товаре| EMPLOYEE
	
	ADMIN--> |Информация о новом пользователе, о новых правах пользователя|id1((Система управления складом WMS))
	id1((Система управления складом WMS)) --> |Информация о пользователях|ADMIN
	
	id4(Интернет магазин)--> |Информация о заказах со списком товаров|id1((Система управления складом WMS)) 
	id1((Система управления складом WMS)) --> |Информация о наличии товаров|id4(Интернет магазин)
	
	id6(Служба доставки)--> |Заявка на сборку заказа|id1((Система управления складом WMS)) 
	id1((Система управления складом WMS)) --> |График погрузки ТС и номер ворот склада|id6(Служба доставки)

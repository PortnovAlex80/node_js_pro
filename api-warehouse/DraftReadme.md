...
rectangle Shop {
  ...
  ' catalog
  usecase (Просмотреть каталог продуктов) as UC3
  usecase (Посмотрнть карточку продукта) as UC4
...
Rel(courier_app, courier_bff, "Изменить статус доставки", "HTTPS")
...
Rel(courier_bff, microservices, "Использует", "HTTPS")
...
Rel(shop_app, auth, "Аутентифициуется", "HTTPS")
Rel(backoffice_web_app, auth, "Аутентифициуется", "HTTPS")
Rel(courier_app, auth, "Аутентифициуется", "HTTPS")
}
``` -->
...
System_Boundary(microservices, "Microservices") {
  ...
  Rel_R(customer, basket, "Изменить состав корзины", "HTTPS")

  !include services/discounts/normal.puml
  Rel_R(basket, discounts, "Проверить наличие скидки", "Sync, gRPC")

  !include services/catalog/normal.puml
  Rel(catalog, basket, "Товар добавлен/изменен", "Async, Kafka")
  Rel_R(customer, catalog, "Найти и выбрать товар", "HTTPS")
  Rel_L(manager, catalog, "Добавить/Изменить товар", "HTTPS")

  !include services/order/normal.puml
  Rel(basket, order, "Корзина оформлена", "Async, Kafka")
  Rel(manager, order, "Получить статус доставки", "HTTPS")
  Rel_D(order,courier,"Назначает заказ")
}
```
...
| Процесс сборки и доставки      | Core          | Gamma    | [order](services/order)         | [C#](https://GitLab.com/microarch-ru/microservices/dotnet/order), [Go](https://GitLab.com/microarch-ru/microservices/dotnet/order)         | Курьеры, процесс досставки                                    |
| Скидки и акции                 | Supporting    | Omega    | [discounts](services/discounts) | [C#](https://GitLab.com/microarch-ru/microservices/dotnet/discounts) | Предоставляет скидку, если состав корзины подпадает под акцию |

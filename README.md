...
# далее черновик, пока не отредактировал. как собственно и все остальное)
# Введение 
## 1.1 Цель документа 
## 1.2 Аудитория 
...
Производительность: калькулятор должен выполнять все операции за менее чем 0.5 секунд. Исключение операции результатом которых , является числа порядка 10^18 и выше.
### 3.2.1 Время отклика 
### 3.2.2 Расход памяти 
## 3.3 Требования к надежности 
### 3.3.1 Ошибки и исключения
Калькулятор при возникновении деления на 0 вывести ошибку о невозможности деления на 0.
При вводе нечислового типа данных, вывести ошибку "Агрумент не соответствует числовому формату"
При ошибке переполнения вывести ошибку: Значение слишком большое для выводы"
...
# Спецификация требований к программному обеспечению (SRS) для командной строки калькулятора
...
- Качественные атрибуты программного обеспечения
- Ограничения
...
Примечание: вышеуказанная структура является лишь образцом и может отличаться в зависимости от конкретных потребностей и требований проекта.

...
| BRULE-02 | Товары на складе должны располагаться в соответствии с их характеристиками и частотой обращения, чтобы максимально использовать полезную площадь склада и время поиска и перемещения товаров | BREQ-02 WMS.УправлениеХранениемТовара |
| BRULE-03 | График сборки заказов должен формироваться автоматически на основе текущей загруженности склада и времени, необходимого для выполнения операций | BREQ-03 WMS.ФормированиеГрафикаСборки |
...
*Карточка проекта используется для снижения уровня сложности реальной системы WMS до практического задания курса NodeJS.*
...
|  Ключевые свойства и возможности (фичи) | Управление каталогом товаров, Приемка/отгрузка товаров, Мониторинг остатков товаров, Управление пользователями и правами |
| Для складских работников  | 1 Приемка товара на склад  |
|   | 2 Инвентаризация |
|   |3 Сборка заказа (future release) |
|Для менеджера склада  | 1 Мониторинг запасов товаро в через REST API и Telegram-bot  | 
...
⚠️Примечание (убрать попозже)
В реальной системе ключевые возможности системы намного сложнее и шире, для учебных целей согласно практическому заданию курса NodeJS реализация ключевых функциональных возможностей имитировано в коде:

| Функциональность | Сущность | Операция |
| -- | -- |-- |
| Приемка товара на склад |  Products| **C**-R-U-D-L 
| Инвентаризация | Products |C-**R-U-D-L** |
| Мониторинг запасов товаров | Products |C-**R**-U-D-**L** |
| Управление доступом пользователей | Users| **C-R-U-D-L** |
...
```mermaid
graph 
    id1((Система управления складом WMS)) -->|Добавление товара| id(Работник склада)
	id(Работник склада)-->|Товар добавлен|id1((Система управления складом WMS))
    id1((Система управления складом WMS)) -->|Товар| Warehouse
    Warehouse -->|Данные о наличии товара|id1((Система управления складом WMS))
    id2(Менеджер склада)
        id3(Администратор)
         id4(Поставщики товара)
```

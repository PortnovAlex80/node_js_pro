...
Краткая суть проекта для экономии времени: в репозитории предоставлена документация на учебный проект WMS, разработанная начиная от бизнес-требований до диаграммы классов и проектирования REST API. Те, представлена проектная работа. Так же в репозитории расположено написанное мною бекенд-приложение, включая RESP API, swagger и Telegram-bot реализующее проектные решения. Те, данное портфолио показывает понимание мною процесса разработки от стадии анализа требований до реализации, включая диаграмму Ганта разработки MPV продукта. Пример учебный. Задача показать трассировку требований в проектное решение, и далее реализации в коде, поэтому многие вещи упрощены, но основные этапы разработки программного обеспечения отображаются.

## Оглавление

ЕСЛИ ВИДИТЕ ЭТО ПРИМЕЧАНИЕ, ЗНАЧИТ ПРОЕКТ АКТИВНЫЙ И ПОСТОЯННО ДОРАБАТЫВАЕТСЯ.

Приложение  завершено. (TypeScript)
Путь /api-warehouse

ОЖИДАЕМЫЙ СРОК ЗАВЕРШЕНИЯ 
1 ЭТАПА - до 10 июня (проектирование документации)  
ЭТАП 2 - рефакторинг кода (приведение к документации) без сроков из-за обучения на другом очень напряженном курсе ([Разработка энтерпрайз приложений на Java Чистая архитектура DDD Event sourcing ](https://howto.stringconcat.com/enterprise)
...
(https://learn.purpleschool.ru/public/products) в рамках которого я выступаю в роли Software engineer, включая роли:
...
Во второй части портфолио будут представлены:
- Event storming(==разработано для контура ИМ ссылку на miro== )
- выделение агрегатов доменных областей и bounded contexts(==разработано для контура ИМ ссылку на miro==)
этап 2:
...
Отчет по складским операциям"| MGR
    ...
    WMS -->|"Каталог товаров,  
Товарная накладная, Заказ на сборку"| EMPLOYEE
...
⚠️Дальнейший раздел касаемо требований носит демонстративный характер и не "причесывался" с ранее разработанными Use cases. В некоторых проектах Use cases могут быть точечно дополнены функциональными требованиями. Могут потребоваться и нефункциональные требования (иногда их называют архитектурные характеристики). 

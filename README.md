...
alt товар найден
    ...
    alt данные введены некорректно
        ...
        S ->> D: Запросить данные о текущем остатке по каждому товару
        D -->> S: Вернуть остатки по приходуемым товарам
        S ->> D: Добавить остаток по каждому приходуемому товару на поступившее количество и сохранить в БД
        S -->> R: Сообщить об успешном завершении операции
    end
else товар не найден
    S -->> R: Предложить ввести новый товар в систему
